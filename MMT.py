...
# Plot the base graph
axisLimit=bit_sec*sample_time/10
plt.ion()
fig = plt.figure(111)
axes = fig.add_subplot(111)  # Add subplot (dont worry only one plot appears)
axes.set_autoscale_on(True)  # enable autoscale
axes.autoscale_view(True, True, True)
axes.set_ylim(bit_sec*sample_time/2-axisLimit, bit_sec*sample_time/2+axisLimit)
...
while time_actual < int_time + delay_time:
    ...
    if count == byte_sec*sample_time - 1:
        ...
        mid_line.append(sample_time * bit_sec / 2)
